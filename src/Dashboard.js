...
class Dashboard extends Component {
  ...
  componentDidMount() {
    ...
    (async () => {
      let response = await fetch(
        `https://excelinsure-backend-229kl.ondigitalocean.app/api/trade/his/${
          this.state.user.user ? this.state.user.user.user._id : ""
        }`
      );
      let data = await response.json();
  ...
  delOrder = (id) => () => {
    (async () => {
      ...
      let response = await fetch(
        `https://excelinsure-backend-229kl.ondigitalocean.app/api/trade/del/${id}`
      );
      let value = id;
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.data !== this.state.history) {
      (async () => {
        let response = await fetch(
          `https://excelinsure-backend-229kl.ondigitalocean.app/api/trade/his/${this.state.user.user.user._id}`
        );
        let data = await response.json();
        this.setState({
          history: data,
  ...
  subDeposite = (e) => {
    ...
    fetch(
      `https://excelinsure-backend-229kl.ondigitalocean.app/api/trade/deposit/${this.state.user.user.user._id}`,
      {
        mode: "cors",
        method: "POST",
        headers: {
